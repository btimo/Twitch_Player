...
class GUI():
    ...
    def create_table(self, object_list, next_list):
        ...
        for elem in object_list:
            ...
            but = tk.Button(self.table_canvas_frame, text=elem.name if isinstance(elem, browse_module.Game) else elem.display_name, image=im, compound=tk.TOP, height=game_img_height + 50,wraplength=100, command=lambda elem=elem : self.browse_channels(elem) if isinstance(elem, browse_module.Game) else self.watch_stream(elem))
        ...
        if next_list != None:
            tk.Button(self.table_canvas_frame, text='Next Games' if isinstance(elem, browse_module.Game) else 'Next Channels',height = 10 , command=lambda: self.browse_games(next_list['offset']) if isinstance(elem, browse_module.Game) 
    ...
    def create_browsing_panel(self):
        # title bar for section title (All games/Game name) and search entry for specific element in the table
        ...
        self.table_canvas = tk.Canvas(self.table_frame, yscrollcommand=yscrollbar.set, bg='black')
        self.table_canvas.grid(row=0, column=0, sticky = tk.N+tk.S+tk.E+tk.W)

        self.table_canvas_frame = tk.Frame(self.table_canvas)

        self.table_canvas.create_window(0, 0, anchor=tk.N + tk.W, window=self.table_canvas_frame)
